...
Ns = 50 # Number of Poisson neurons (source)
...
############################## Motor cortex ################################
mc_p = nest.Create("diff_neuron", Nm)
nest.SetStatus(mc_p, {"kp": gain, "pos": True, "buffer_size":buf_size, "base_rate": bas_rate_motor})

mc_n = nest.Create("diff_neuron", Nm)
nest.SetStatus(mc_n, {"kp": gain, "pos": False, "buffer_size":buf_size, "base_rate": bas_rate_motor})
...
#print(1000*np.sum(count_p)/(Nm*time_span))
...
axs[3].bar(bins[:-1], rate_p, width=bins[1] - bins[0],color='r')
axs[3].plot(bins[:-1],rate_sm_p,color='k')
axs[4].bar(bins[:-1], rate_n, width=bins[1] - bins[0],color='b')
